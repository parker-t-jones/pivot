...
### A notification tapped from a killed app opens Home and silently drops the switch (Sprint 10 Track B discovery) — fix before v1 launch
...
**Root cause:** `NotificationResponseHandler` uses `Notifications.addNotificationResponseReceivedListener`, which only delivers responses arriving *after* subscription and never replays the tap that launched the app. The component mounts inside `AppGroupLayout`, behind `PushPermissionProvider`, `LeaguesGateProvider`, `SwitchingProvider` and the `(app)` route group's auth gate — so on a cold start the launching response is delivered before the listener exists and is dropped. The APIs that survive this are `useLastNotificationResponse()` / `getLastNotificationResponseAsync()`, which return the launching response regardless of subscription timing; neither is used anywhere in the client.

**Impact:** The killed-app case is arguably the *most* likely state for a real notification tap — a user reacting to a red-zone alert hours after last opening the app. Section 6's whole delivery premise assumes a tap results in a switch. This escaped Simulator testing because those runs always tapped from a backgrounded app, and it escaped Sprint 10 Track B (B1) because that phase verified *delivery* (did the notification arrive) rather than the response path from cold start. Not fixed in this pass to keep the deep-link data fix isolated; it needs a device test per app state (foreground / background / killed), which is its own verification matrix.
